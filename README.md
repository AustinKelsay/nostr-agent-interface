...
This project starts as a direct fork/clone baseline of `nostr-mcp-server`, then layers additional transports so the same tool surface is available through:

1. MCP (`stdio`) for MCP-compatible clients
2. CLI for command-line usage
3. HTTP API for programmatic integration

## Unified Interface Commands

```bash
# MCP mode (original behavior)
...
2. `nostr-agent-interface cli call <toolName> [jsonArgs] --json`
3. `echo '{"input":"...","targetType":"hex"}' | nostr-agent-interface cli call convertNip19 --stdin --json`

API endpoints:

1. `GET /health`
2. `GET /tools`
3. `POST /tools/:toolName` with JSON body for tool arguments

API error responses are standardized and include request correlation IDs:
...
```json
{
  "error": {
    ...
    "requestId": "b62495a1-0bc9-4b81-a91f-5d19d2435320"
  }
}
```
...
1. `code`: stable machine-readable category (for example `invalid_json`, `invalid_request`, `not_found`, `internal_error`)
2. `message`: human-readable summary
3. `details`: optional context payload
4. `requestId`: per-request ID (also emitted in `x-request-id` response header)

## Tool Manifest

Tool schemas are captured in a versioned artifact:
...
Model-facing integration docs live in `llm/`:
...
Testing and parity guidance lives in:
...
It runs build, core tests, interface parity tests, and docs-link checks.

## Configuration

Use `.env.example` as the baseline for environment configuration.

Precedence:

1. Explicit tool/API/CLI args
2. Environment variables
3. Built-in defaults

Primary environment variables:

1. `NOSTR_DEFAULT_RELAYS`: global default relays when tools do not receive a `relays` argument
2. `NOSTR_AGENT_API_HOST`: API host fallback
3. `NOSTR_AGENT_API_PORT`: API port fallback
4. `NOSTR_MCP_COMMAND`: CLI/API MCP child process command override
5. `NOSTR_MCP_ARGS`: CLI/API MCP child process arguments override
